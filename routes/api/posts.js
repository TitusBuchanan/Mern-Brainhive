...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      //epic: create a new post and send it to the requester
      const profile = Profile.findOne({ user:req.user.id });
      const profileId =  profile._id;
      
      //find profile with userId so we can ectract the profile id
      //attach the profileId to the postData
      const postData = {...req.body};
      postData.poster = profileId
      
      //create a new post and send it to requester
